...
class Car {
  ...
  drive(distance){
    const drivenMile = this.tank * this.milesPerGallon;
    if(distance <= drivenMile){
      this.odometer = this.odometer + distance;
      this.tank = this.tank - (distance / this.milesPerGallon);
    }else{
      this.odometer = this.odometer + drivenMile;
      this.tank = 0;
...
class ProjectManager extends Instructor{
  constructor ({name, age, location, specialty, favLanguage, catchPhrase, gradClassName, favInstructor}){
   ...
   standUp(channel){
     return `${this.name} annouces to ${channel}, @channel study times!`
   }
